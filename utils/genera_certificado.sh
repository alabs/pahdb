...
openssl genrsa -des3 -out pahdb-passwd.key -passout $PASSWD 4096

# Generamos la solicitud de firma de certificado (CSR)
openssl req -new -key pahdb-passwd.key -out pahdb.csr -passin "$PASSWD" -subj /CN="$DOMINIO"

# Generamos la clave privada sin contraseña: pahdb.key
openssl rsa -in pahdb-passwd.key -out pahdb.key -passin "$PASSWD"
